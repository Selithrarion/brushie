<template>
	<div class="fixed top-4 left-1/2 z-10 flex -translate-x-1/2 flex-col items-center justify-center">
		<div class="flex w-fit gap-2 rounded-lg border-white/30 bg-white/20 p-4 shadow-lg backdrop-blur-md">
			<UiButton
				v-for="item in tools"
				:key="item.name"
				:class="[
		...
		<div
			v-if="selected === ToolName.SELECT || selected === ToolName.LINE || selected === ToolName.ARROW || selected === ToolName.TEXT"
			class="mt-2 text-center text-xs text-gray-500 select-none"
		>
			<div v-if="selected === ToolName.SELECT">hold mouse wheel or spacebar while dragging, or use the hand tool</div>
			<div v-else-if="selected === ToolName.LINE || selected === ToolName.ARROW">press ESC to cancel drawing</div>
