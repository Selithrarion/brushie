...
import { generateTestMeshes } from '@/features/render/utils/generateTestMeshes.ts'
...
function measureCounts() {
	...
	props.scene.traverse((obj) => {
		const isArrow = Boolean(obj.userData.mainMesh)
		const isLine = (obj as THREE.Line).isLine && !obj.userData?.mainMesh
		const isMesh = (obj as THREE.Mesh).isMesh
		const isPencil = (obj as Line2).isLine2
		if (isArrow) {
			const main = obj.userData.mainMesh as THREE.Line
			mesh++
			if (main.geometry && main.geometry.attributes.position) {
				vertex += main.geometry.attributes.position.count
			}
		} else if (isPencil) {
			mesh++
			...
			mesh++
			const g = (obj as THREE.Line).geometry
...
onMounted(() => {
	generateTestMeshes(props.scene)
	requestAnimationFrame(update)
})
</script>
